...
| organize | `src/organize/` | AI-powered semantic directory structuring for notes | `OrganizeModule`, `ContentAnalyzer`, `DirectoryMatcher`, `buildSummaryPath`, `renderOrganizeSettings`, types (`OrganizeProposal`, `OrganizeSnapshot`, `OrganizeResult`, `ContentAnalysis`, `DirectoryScore`, `NoteTopic`, `OrganizeAction`, `OrganizeProposalStatus`) |
| deep-dive | `src/deep-dive/` | Recursive topic extraction and child note generation | `DeepDiveModule`, `buildDeepDivePath`, `renderDeepDiveSettings`, syllabus-navigator helpers (`computeTraversalOrder`, `buildNavigationContext`, `renderNavigationBlock`, `renderSyllabusContent`, `buildTreeFromNodes`, `syllabusTitle`, `syllabusPath`, `injectNavigationBlock`), types (`DeepDiveProposal`, `DeepDiveRun`, `ExtractedTopic`, `QualityScore`, `DeepDiveProposalStatus`, `DeepDiveRunStatus`, `TraversalNode`, `NavigationContext`) |
...
| shared | `src/shared/` | AI client (multi-modal + opt-in response cache), file utils, validation, notifications, callouts, frontmatter, checkpoints, per-note operation queue, credential metadata + validation, secret redaction, settings migrations + defaults merge, data-folder migration, content hashing, untrusted-content wrapping, review-toast gate, update check, title predicates, feature-module lifecycle contract (#504), no-speech outcome (#524), cache-hit reporting (#527) | `AIClient`, `AIRequestOptions` (`{ bypassCache?; onCacheHit? }`), `NotificationManager`, `CheckpointManager`, `NoteOperationQueue`, `TranscriptCache`, `validateCredentials`, `PROVIDER_METADATA`, `decorateCredentialField`, `redactSecrets`, `redactError`, `reviewAction`, `migrateSettings`, `deepMergeSettings`, `migrateDataFolder`, `hashString`/`contentKey`, `wrapUntrusted`, `findAvailableVaultPath`, `ModuleDeps`, `FeatureModule`, `FeatureSettingsKey`, `UpdateChecker`, `isNewerVersion`, `isUntitled`, `isGenericTitle`, `NoSpeechDetectedError`, `isNoSpeechError`, `hasSpeechContent`, `isWorthPostProcessing`, `noSpeechNotice`, `NO_SPEECH_MESSAGE`, `MIN_TRANSCRIPT_CHARS_FOR_AI`, `CacheUse`, `mergeCacheUse`, `transcriptCacheUse`, `trackAiCache`, `withCacheReport` (`usedCache` is module-internal), file/validation utils, callout registry, id-utils |
...
| properties-fold | `src/properties-fold/` | Auto-fold a note's Properties panel on open (#381) | `registerPropertiesAutoFold`, `applyPropertiesFold`, `foldActiveNoteProperties`, `foldPropertiesIn`, `METADATA_CONTAINER_SELECTOR`, `PROPERTIES_COLLAPSED_CLASS`, `PROPERTIES_COLLAPSE_INDICATOR_SELECTOR` |
...
```ts
SynapseSettings {
  settingsVersion: number                           // #93; persisted schema version, stamped to CURRENT_SETTINGS_VERSION (3, shared/settings-migrations.ts:25) on save; drives the migration runner
  ai: AISettings {
