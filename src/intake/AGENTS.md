...
Exported from `index.ts` (`index.ts:24-32`, `index.ts:725`):

```ts
// index.ts:69 — class IntakeModule
class IntakeModule {
  constructor(moduleDeps: ModuleDeps, deps: IntakeDeps)   // index.ts:90; keeps plugin/getSettings/notifications from the bundle, not checkpointManager/registrar/noteQueue
  onload(): Promise<void>    // index.ts:97 — registers vault create+modify listeners when intake.enabled; arms the startup catch-up scan (#462)
  onunload(): void           // index.ts:120 — clears the catch-up timer, all debounce timers, pending/inFlight/failureNotified sets
}
...
| Body shape | `classifyUrl` type | Route `kind` | Branch (execute, index.ts:434) |
|------------|--------------------|--------------|--------------------------------|
...
vault create/modify event
  --> handleEvent (index.ts:186): cheap sync guards, cheapest-first:
        is TFile && .md  -->  intake.enabled
        -->  isInIntakeFolder (index.ts:251; excludes capture-log subfolder)
             OR isAdoptionCandidate (index.ts:229; #455)
        -->  not isPathExcluded(...,'intake',...)  -->  not inFlight
  --> scheduleFlush(path, extraDelayMs = 0) (index.ts:292): per-path debounce, resets timer on every event
        settleWindowMs (index.ts:315) = intake.settleSeconds * 1000
        (fallback DEBOUNCE_MS=5000ms when missing/not a positive number, index.ts:40)

startup catch-up scan (#462), armed by onload via workspace.onLayoutReady + CATCHUP_DELAY_MS (index.ts:112)
  --> catchUp (index.ts:135): vault.getMarkdownFiles()
        isInIntakeFolder AND not isPathExcluded AND not pending AND not inFlight
        sort by stat.mtime ascending --> skip stamped (isFileProcessed, index.ts:166)
        first CATCHUP_MAX_NOTES=10 --> scheduleFlush(path, i * CATCHUP_STAGGER_MS=2000)
  --> flush(path) (index.ts:328) after the note is quiet for the full window:
        path outside intake folder --> maybeAdoptCapture(file) (index.ts:380), return
        read + parseFrontmatter
        idempotency guard (isProcessed, index.ts:417): skip if SYNAPSE_PROCESSED_FLAG truthy
        dispatcher.route(file, parsed) --> execute(file, route)
  --> execute (index.ts:434):
        ...
        article:       fetchArticleContent(url) --> appendArticleContent (index.ts:481) --> deps.fireOnFile
...
- `catchUp` (index.ts:135) re-checks `intake.enabled`, enumerates `vault.getMarkdownFiles()`, keeps notes inside the intake folder (capture-log subfolder excluded via `isInIntakeFolder`) that are not path-excluded and not already `pending`/`inFlight` (an event-scheduled debounce is never reset), sorts oldest-first by `stat.mtime`, skips stamped notes (`isFileProcessed`, index.ts:166; a read failure counts as processed), and schedules at most `CATCHUP_MAX_NOTES` = 10 through the normal `scheduleFlush` path with `i * CATCHUP_STAGGER_MS` (2000ms) extra delay per note so flushes fan out instead of bursting AI calls.
...
Failure notices are once-per-session per path (`reportFailure`, index.ts:362; `failureNotified` set): the first failure toasts via `notifications.notifyError`, repeats only `console.warn`. A `modify` event for the path clears the entry so edited content gets a fresh toast.
...
- `isAdoptionCandidate` (index.ts:229): `adoptSharedCaptures === true` AND event kind is `create` (never `modify`) AND path has no `/` (vault root) AND `intakeFolder` is non-blank.
- `maybeAdoptCapture` (index.ts:380): re-checks the toggle; skips already-stamped notes; requires `dispatcher.bareUrl(body)` non-null AND `classifyUrl(url).type !== 'unknown'`; then `moveNote(file, intakeFolder)` and `scheduleFlush(file.path)` (a rename fires no create/modify event, so the adopted note is re-queued explicitly).
- Anything else (prose, multiple URLs, unclassifiable link) is left where the user created it.
- Failure → `notifications.notifyError('Could not adopt shared capture ...')` (index.ts:404); the note is not moved.
...
When `intake.captureLog` is true and a processed note actually left the intake folder (`movedOutOfIntake`, index.ts:549), a dated breadcrumb is written to `<intakeFolder>/<captureLogFolder>/<YYYY-MM-DD> — <title>.md` (default subfolder `_captured`). The capture-log subfolder is excluded from the watcher (`isInIntakeFolder`, index.ts:251; `captureLogPath`, index.ts:277) and breadcrumbs are stamped `synapse-processed: true` (defense-in-depth) so they are never re-ingested.

Collision policy (#227, `resolveBreadcrumbPath`, index.ts:652): two distinct notes that sanitize to the same dated title get a uniqueness suffix (` (2)`, ` (3)`, ...). Re-processing the same note overwrites its own breadcrumb idempotently (`breadcrumbTargets`, index.ts:684).
...
| Condition | Handling |
|-----------|----------|
| File vanished before flush | `flush` returns silently (index.ts:330-332) |
| Note already processed | `flush` returns before routing (idempotency, index.ts:347) |
| Processing throws (transcription/fetch/pipeline) | Caught in `flush`; note left un-stamped (retriable); surfaced via `reportFailure` (index.ts:362) — `notifications.notifyError` once per session per path, then `console.warn`; cleared by a `modify` event |
...
| Adoption read/move fails | Caught in `maybeAdoptCapture`; `notifications.notifyError` (index.ts:404); note untouched |
| Breadcrumb read fails during collision check | Treated as "not ours" → falls through to a suffix rather than clobbering (`breadcrumbTargets`, index.ts:684) |
| Empty/whitespace `intakeFolder` | Watches nothing; `isInIntakeFolder` returns false (index.ts:252-254); adoption disabled; catch-up scan schedules nothing |
| Catch-up read of a candidate fails | Treated as processed (`isFileProcessed`, index.ts:166); skipped silently |
...
| `vault.getMarkdownFiles`, `workspace.onLayoutReady` | `obsidian` (runtime `app` surfaces used by the catch-up scan) | `index.ts:142`, `index.ts:112` |
