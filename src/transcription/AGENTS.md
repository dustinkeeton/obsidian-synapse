...
interface UrlTranscriptionStrategy {
  readonly id: string
...
class UrlTranscriptionRouter {
  constructor(strategies: UrlTranscriptionStrategy[])           // order IS the tier order
  transcribe(url: string, opts?: UrlTranscriptOptions): Promise<UrlTranscript>
}
class NoTranscriptionPathError extends Error { }                // every tier declined; message is platform-aware (mobile -> desktop/sync handoff)
function buildUrlTranscriptBlock(result: UrlTranscript, url: string, embedInNote: boolean, timeRange?: TimeRange): string

// caption-strategy.ts — tier 1: YouTube captions over HTTP (free, mobile-capable)
interface ProcessedTranscript { text: string; reformatted?: boolean; schemaId?: string }
type ProcessTranscript = (raw: string) => Promise<ProcessedTranscript>
class CaptionStrategy implements UrlTranscriptionStrategy {
  readonly id = 'captions'
  constructor(getSettings: () => SynapseSettings, postProcess: ProcessTranscript)   // postProcess injected to stay decoupled from AudioModule
}

// local-extraction-strategy.ts — tier 2: desktop yt-dlp/ffmpeg
type LocalExtractionDelegate = (url: string, opts: UrlTranscriptOptions) => Promise<TranscriptionResult & { videoVaultPath?: string }>
class LocalExtractionStrategy implements UrlTranscriptionStrategy {
  readonly id = 'local-extraction'
  constructor(delegate: LocalExtractionDelegate)                // delegate is VideoModule.processUrl, wired in main.ts
  // canHandle: Platform.isDesktop && isSupportedUrl(url)
}

// youtube-captions.ts
interface YouTubeTranscript { text: string; language: string; /* ...autoGenerated, title */ }
function fetchYouTubeTranscript(url: string, preferredLanguages: string[]): Promise<YouTubeTranscript | null>
...
- Tier order is the array order handed to `UrlTranscriptionRouter`; main.ts always puts `CaptionStrategy` first and appends `LocalExtractionStrategy` only where `VideoModule` exists (desktop). A strategy returning `null` falls through; only a real failure throws. All tiers declining raises `NoTranscriptionPathError`
