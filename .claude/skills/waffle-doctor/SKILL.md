...
```bash
npx --yes github:dustinkeeton/wafflestack#v0.16.1 doctor
```
...
```bash
npx --yes github:dustinkeeton/wafflestack#v0.16.1 doctor --verify-render
```
...
- **version-skew note** — doctor mentions when the lock was written by a different toolkit
  version than the one that just ran. That is informational. Since #469 `github:dustinkeeton/wafflestack#v0.16.1`
