...
4. **Seat constraint.** The seat an orchestrator runs from decides what it may do (verified against the live harness, 2026-09-18). From a **named** agent's seat a named spawn is rejected — `Teammates cannot spawn other teammates — the team roster is flat` — so omit `name:` and address the agent by the `agentId` the spawn returns; `SendMessage` and `TaskStop` both accept it in place of a name. From an **unnamed** subagent's seat a named spawn is accepted and `SendMessage(to: <name>)` reaches it. From **any** spawned seat, teardown is impossible: `shutdown_request` messages and `TaskStop` are acts of the main session and are rejected from a spawned agent. An orchestrator running from a spawned seat therefore hands its agent names back to the session that spawned it — that session performs the teardown — rather than reporting a teardown it cannot perform.
